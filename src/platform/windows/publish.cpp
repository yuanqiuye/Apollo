  ...
  public:
    mdns_registration_t():
      ...
      if (service(true, existing_instance)) {
        BOOST_LOG(error) << "Unable to register " PROJECT_NAME " mDNS service"sv;
        return;
      }

      BOOST_LOG(info) << "Registered " PROJECT_NAME " mDNS service"sv;
    }

    ~mdns_registration_t() override {
      if (existing_instance) {
        if (service(false, existing_instance)) {
          BOOST_LOG(error) << "Unable to unregister " PROJECT_NAME " mDNS service"sv;
          return;
        }

        BOOST_LOG(info) << "Unregistered " PROJECT_NAME " mDNS service"sv;
      }
    }
